...
- **新模块 `writes.py`**：纯逻辑、不发请求、可完全离线测试
...
- 假服务 `tests/fake_yearning.py` 复刻真实服务的怪癖：错误子路径返回字符串 `"Illegal"`、
  登录在根路径、`fetch/perform` 带密码哈希、`fetch/source` 空参数返回空响应体、
  数据源名带前导空格；并覆盖 PUT / DELETE 路由与写操作留痕断言
- **153 个离线测试用例**：`tests/test_writes.py` 30 个纯逻辑用例；
  `test_tools.py` 覆盖 14 个工具，每个拒绝用例都断言**请求确实没发出去**
